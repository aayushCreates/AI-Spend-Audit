...
const AVAILABLE_TOOLS = [
  ...
  },
  {
    ...
    plans: [{ id: "api_direct", name: "API Direct (Pay as you go)", price: 0 }],
  },
  {
    id: "openai_api",
    name: "OpenAI API",
            ...
            >
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-lg ${tool.iconColor}`}
                  >
                    {tool.name.charAt(0)}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div
                    className={`w-11 h-6 rounded-full relative transition-colors duration-300 ${isActive ? "bg-green-500" : "bg-gray-800"}`}
                  >
                    <div
                      className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 ${isActive ? "left-6" : "left-1"}`}
                    ></div>
                  </div>
                </div>
              </div>
              ...
              {isActive && (
                <div className="p-4 border-t border-white/5 flex flex-col gap-4 animate-in fade-in slide-in-from-top-4 duration-300">
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1.5">
                      Actual bill override (optional)
                    </label>
                    <input
                      type="text"
                      value={state.override}
                      onChange={(e) =>
                        updateTool(tool.id, "override", e.target.value)
                      }
          ...
          <div className="bg-[#0A0A0A] border border-white/10 rounded-xl p-4 flex justify-between items-center shadow-2xl">
            <div>
              ...
              <p className="text-xs text-gray-400">
                Based on plans + seats you selected.
              </p>
            </div>
